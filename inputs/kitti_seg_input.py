...
def _make_data_gen(hypes, phase, data_dir):
    ...
    colors = [np.array(c['colors']) for c in hypes['classes']]
    ...
    for image, gt_image in data:
        ...
        gts = [np.all(gt_image == color, axis = 2) for color in  colors]


        assert(gts[0].shape == gts[1].shape)
        shape = gts[0].shape
        gts = [gt.reshape(shape[0], shape[1], 1) for gt in gts]
...
#        gt_image = np.concatenate((gt_bg, gt_road), axis=2)
        gt_image = np.concatenate(gts, axis=2)
