...
class FCN8VGG:
    ...
    def build(self, rgb, train=False, num_classes=20, random_init_fc8=False,
        ...
        posLR = np.zeros((1, 480, 720, 1), np.float32)
        posTD = np.zeros((1, 480, 720, 1), np.float32)
        posIO = np.zeros((1, 480, 720, 1), np.float32)
        posRT = np.zeros((1, 480, 720, 1), np.float32)
        for f in range(0,1):
            for w in range(0,480):
                for d in range(0,720):
                    for c in range(0,1):
                        posLR[f][w][d][c] = w
                        posTD[f][w][d][c] = d
                        posIO[f][w][d][c] = abs(w - 240) + abs(d - 360)
                        posRT[f][w][d][c] = 100 * (w - 240) / (d - 360 + 0.5)
        addInf_const =  np.concatenate((posLR, posTD, posIO, posRT), axis = 3)
        print (addInf_const.shape)
        addInf = tf.nn.max_pool(addInf_const, ksize=[1, 24, 24, 1], strides=[1, 24, 24, 1],
                              padding='SAME')
        print (addInf)
        print (self.pool3)
        print (self.pool3[:,:,:,:256-4])

#        self.conv4_1 = self._conv_layer(self.pool3, "conv4_1")

        self.conv4_1  = tf.concat([self.pool3[:,:,:,:256-4], addInf], 3)
        print (self.conv4_1)
        self.conv4_1  = tf.concat([self.conv4_1, self.conv4_1], 3)
        print (self.conv4_1)

        self.conv4_1 = tf.Print(self.conv4_1, [tf.shape(self.conv4_1)],
                               message='Shape of conv4_1: ',
                               summarize=4, first_n=1)
